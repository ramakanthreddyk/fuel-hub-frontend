...
import { EntityStatus } from '@/api/api-contract';
        ...
        {filteredStations.length === 0 ? (
          <div className="bg-white/80 backdrop-blur-sm rounded-2xl shadow-xl p-12 border-0">
            <EmptyState
              ...
              onAction={() => navigate('/dashboard/stations/new')}
            />
          </div>
        ) : (
...
function StationCardWithData({ station, onView, onDelete }: Readonly<{
  station: Station;
  ...
  const todaySales = React.useMemo(() => stationData?.todaySales || 0, [stationData]);
  const todayTransactions = React.useMemo(() => stationData?.todayTransactions || 0, [stationData]);
  const activePumps = React.useMemo(() => pumps.filter(p => p.status === 'active').length, [pumps]);
  ...
  return (
    <ModernStationCard
      ...
      onView={onView}
      onDelete={onDelete}
      fuelPrices={fuelPrices as { fuelType: string; price: number }[]}
      pumps={pumps}
