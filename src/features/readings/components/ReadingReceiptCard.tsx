...
import { getStatusConfig } from '@/utils/reading-status-config';
import { calculateVolumeDifference, isLargeVolumeJump } from '@/utils/reading-volume-utils';
...
export function ReadingReceiptCard({ reading, onView, onEdit }: ReadingReceiptCardProps) {
  ...
  const difference = calculateVolumeDifference(currentReading, previousReading);
  const isLargeJump = isLargeVolumeJump(difference, previousReading);
