...
export function StationCard({ station, onView, onDelete }: Readonly<StationCardProps>) {
  ...
  return (
    <div className="group relative bg-card border rounded-2xl shadow-sm hover:shadow-lg transition-all duration-300 overflow-hidden transform hover:scale-[1.02] hover:-translate-y-1">
      ...
      {/* Status Indicator */}
      {(() => {
        let statusClass = 'bg-red-500 shadow-lg shadow-red-500/30';
        if (station.status === 'active') statusClass = 'bg-green-500 shadow-lg shadow-green-500/30';
        else if (station.status === 'maintenance') statusClass = 'bg-orange-500 shadow-lg shadow-orange-500/30';
        return (
          <div className={`absolute top-4 right-4 w-3 h-3 rounded-full ${statusClass} animate-pulse`}></div>
        );
      })()}
