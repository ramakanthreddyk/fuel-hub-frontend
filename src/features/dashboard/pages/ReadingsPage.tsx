...
import { Gauge, AlertTriangle, Plus, FileText, Eye, TrendingUp, Activity, Zap, DollarSign } from 'lucide-react';
...
export default function ReadingsPage() {
  ...
  // Validate readings data strictly by contract
  const validatedReadings = readingsArray.filter(reading => {
    return (
      typeof reading.id === 'string' &&
      typeof reading.nozzleId === 'string' &&
      typeof reading.reading === 'number' &&
      typeof reading.recordedAt === 'string'
    );
  });
  
  // Enrich readings for UI only, derive pumpId, fuelType, pricePerLitre from related objects
  const enrichedReadings = validatedReadings.map(reading => {
    ...
    const nozzleNumber = nozzle?.nozzleNumber || 'Unknown';
    const pumpName = pump?.name || 'Unknown Pump';
    const stationName = station?.name || 'Unknown Station';
    const pumpId = pump?.id;
    const fuelType = nozzle?.fuelType;
    const pricePerLitre = nozzle?.pricePerLitre;
    const volume = reading.previousReading !== undefined ? reading.reading - reading.previousReading : undefined;
    const amount = volume !== undefined && pricePerLitre !== undefined ? volume * pricePerLitre : undefined;
    return {
      ...reading,
      nozzleNumber,
      pumpName,
  ...
  const weekReadings = enrichedReadings.filter(r => r.recordedAt && new Date(r.recordedAt) >= weekAgo).length;
  ...
  // Enrich alerts for UI
  const enrichedAlerts = pendingAlerts.map(alert => {
    // Use alert.id as nozzleId (contract)
    const nozzle = nozzles.find(n => n.id === alert.id);
    const pump = pumps.find(p => p.id === nozzle?.pumpId);
    const station = stations.find(s => s.id === pump?.stationId);
    return {
      ...alert,
      stationName: station?.name || 'Station',
      nozzleNumber: nozzle?.nozzleNumber || 'N/A',
    };
  });
              ...
              <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                {lastReadings.map((reading) => (
                  <ReadingCard
                    key={reading.id}
                    reading={reading}
                    actions={{ onView: (id) => navigate(`/dashboard/readings/${id}`) }} // Provide actions prop
                  />
                ))}
              </div>
            );
                            ...
                            <div>
                              ...
                              {(() => {
                                ...
                                if (reading.amount !== undefined && reading.amount !== null) {
                                  amountDisplay = formatCurrencyMobile(reading.amount);
                                } else if (reading.volume !== undefined && reading.pricePerLitre !== undefined) {
                                  amountDisplay = formatCurrencyMobile(reading.volume * reading.pricePerLitre);
                                } else {
                                  amountDisplay = 'N/A';
                                }
                                return (
                      ...
                      <TableRow key={reading.id} className="hover:bg-gray-50 transition-colors">
                        ...
                        <TableCell className="font-mono text-green-700 font-semibold">
                          {(() => {
                            let amountDisplay;
                            if (reading.amount !== undefined && reading.amount !== null) {
                              amountDisplay = formatCurrency(reading.amount);
                            } else if (reading.volume !== undefined && reading.pricePerLitre !== undefined) {
                              amountDisplay = formatCurrency(reading.volume * reading.pricePerLitre);
                            } else {
                              amountDisplay = 'N/A';
                            }
