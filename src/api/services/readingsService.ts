  ...
  getReadings: async (options?: { nozzleId?: string; limit?: number }): Promise<Reading[]> => {
    try {
      ...
      readingsArray = readingsArray.map((r: any) => ({
        ...
        previousReading: r.previousReading ?? r.previous_reading ? Number(r.previousReading ?? r.previous_reading) : undefined,
        ...
        recordedBy: r.recordedBy || r.recorded_by || r.attendantName || r.attendant_name || 'Unknown',
